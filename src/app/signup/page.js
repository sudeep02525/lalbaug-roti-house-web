...
import { ArrowRight, User, Mail, Lock, Phone, Loader2, Leaf } from 'lucide-react'
...
function SignupContent() {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setOtpMessage('')
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/users/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: formData.email, otp, name: formData.name, password: formData.password, phone: formData.phone }),
      })
      const data = await res.json()
  ...
  return (
    <div className="h-screen flex bg-white relative overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-1/2 h-full relative bg-[#114D3C] overflow-hidden p-12 flex-col justify-between">
        ...
        <div className="relative z-10 text-white/80 self-end font-bold text-sm tracking-widest uppercase" style={{ fontFamily: "var(--font-outfit)" }}>Lalbaug Roti House</div>
      ...
      <div className="w-full lg:w-1/2 h-full overflow-y-auto relative z-10 custom-scrollbar">
        <div className="min-h-full flex items-center justify-center p-6 sm:p-12 lg:p-12 xl:p-16">
          ...
          <Link href="/" className="inline-flex mb-6 lg:mb-8 transform hover:-translate-x-1 transition-transform">
            <img src="/logo.jpeg" alt="Logo" className="h-10 lg:h-12 w-auto object-contain rounded-xl shadow-sm border border-[#EAE5D9]" />
          </Link>
          <h1 className="text-3xl lg:text-4xl font-bold text-[#114D3C] mb-2" style={{ fontFamily: "var(--font-playfair)" }}>{step === 'form' ? 'Create Account' : 'Verify OTP'}</h1>
          <p className="text-[#73706A] mb-6" style={{ fontFamily: "var(--font-outfit)" }}>
            {step === 'form' ? 'Join Lalbaug Roti House for the fastest checkout and fresh daily meals.' : `Enter the OTP sent to ${formData.email}`}
          </p>

          {/* Global messages */}
          {otpMessage && (
            <div className="bg-green-50 text-green-700 p-4 rounded-2xl text-sm font-medium mb-6 flex items-center border border-green-100">
          ...
          <p className="mt-6 text-center text-[#73706A] text-sm font-medium" style={{ fontFamily: "var(--font-outfit)" }}>
            Already have an account?{' '}
            <Link href={`/login?redirect=${encodeURIComponent(redirect)}`} className="text-[#16A34A] font-bold hover:underline">Log in here</Link>
          </p>
        </div>
      </div>
      </div>
