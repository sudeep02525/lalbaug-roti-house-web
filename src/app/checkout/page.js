...
export default function CheckoutPage() {
  ...
  const { items, subtotal, calculateItemTotal, clearCart, mounted } = useCart()
  ...
  return (
    <div className="min-h-screen bg-[#FAF5E9] pb-24 font-outfit selection:bg-[#8B5E3C] selection:text-white">
      ...
      {/* Premium Header */}
      <div className="bg-[#FDFBF7] border-b border-[#E8E1D5] sticky top-0 z-40 shadow-[0_2px_15px_rgba(139,94,60,0.05)] backdrop-blur-md bg-[#FDFBF7]/90">
        ...
        {step === 1 ? (
                        ...
                        {selectedAddressIndex === index && (
                          <div className="animate-fade-in">
                            <button
                              ...
                              className="w-full py-4 bg-[#16A34A] text-white rounded-xl font-bold text-lg hover:bg-[#15803d] transition-all shadow-[0_8px_20px_rgba(22,163,74,0.25)] flex items-center justify-center gap-2 hover:-translate-y-0.5"
                            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
                ...
                <div className="lg:pl-4 space-y-8 flex flex-col justify-center min-h-[650px]">
                  <div className="flex items-center justify-between">
                    <div>
                      <h1 className="text-4xl lg:text-5xl font-bold text-[#1A4D2E] font-playfair mb-3">New Address</h1>
                      <p className="text-[#8B5E3C] text-lg">Enter your details to finalize delivery.</p>
                    </div>
                  ...
                  <div className="bg-[#FDFBF7] p-8 lg:p-10 rounded-[2.5rem] border border-[#E8E1D5] shadow-sm">
                    ...
                    <div className="mt-10">
                      <button
                        ...
                        className="w-full flex items-center justify-between px-8 py-5 bg-[#8B5E3C] text-white rounded-2xl font-bold text-lg hover:bg-[#734A2E] transition-all duration-300 shadow-[0_10px_30px_rgba(139,94,60,0.25)] hover:-translate-y-1 hover:shadow-[0_15px_35px_rgba(139,94,60,0.35)] group"
                      >
          ...
          <div className="animate-fade-in grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            ...
            <div className="lg:col-span-7 xl:col-span-8 space-y-6">
              <div className="flex items-center justify-between mb-8">
                <h1 className="text-4xl lg:text-5xl font-bold text-[#1A4D2E] font-playfair">Review Order</h1>
                <span className="bg-[#FDFBF7] text-[#8B5E3C] font-bold px-4 py-2 rounded-xl text-sm border border-[#E8E1D5]">
                    ...
                    return (
                      <div key={item.cartItemId} className={`flex gap-6 pb-8 ${index !== items.length - 1 ? 'border-b border-[#E8E1D5]' : ''}`}>
                        ...
                        <div className="flex-1 flex flex-col justify-center">
                          ...
                          {item.variant && <p className="text-xs sm:text-sm text-[#8B5E3C] font-bold uppercase tracking-widest mb-1">{item.variant.name}</p>}
                          {item.addons?.length > 0 && (
                            <p className="text-xs sm:text-sm text-[#8B5E3C]/70 mb-2 font-medium">
                              With: {item.addons.map(a => a.name).join(', ')}
                            </p>
                          )}
                          <p className="text-[#1A4D2E]/60 font-bold text-sm tracking-widest uppercase mt-auto">Qty: {item.quantity}</p>
                        </div>
                        <div className="flex flex-col items-end justify-center">
                          {originalTotal > itemTotal && (
                            <span className="text-sm text-[#8B5E3C]/70 line-through mb-1">₹{originalTotal}</span>
                          )}
                          <span className="font-bold text-2xl sm:text-3xl text-[#8B5E3C] font-outfit">₹{itemTotal}</span>
                        </div>
                      </div>
                    )
                  })}
            ...
            <div className="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-32">
              <div className="bg-[#FDFBF7] rounded-[2.5rem] p-8 lg:p-10 text-[#1A4D2E] shadow-sm relative overflow-hidden border border-[#E8E1D5]">
                ...
                <h3 className="text-3xl font-bold mb-8 font-playfair relative z-10 text-[#8B5E3C]">Payment Summary</h3>
                
                <div className="space-y-5 relative z-10 font-outfit">
                  <div className="flex justify-between text-[#73706A] pb-5 border-b border-[#E8E1D5]">
                    <span className="text-lg font-medium">Subtotal</span>
                    <span className="font-bold text-[#1A4D2E] text-xl">₹{subtotal}</span>
                  </div>
                  <div className="flex justify-between text-[#73706A] pb-5 border-b border-[#E8E1D5]">
                    <span className="text-lg font-medium">Delivery Fee</span>
                    <span className="font-bold text-[#1A4D2E] text-xl">₹{deliveryCharge}</span>
                  </div>
                  <div className="flex justify-between items-end pt-4 mb-6">
                    <span className="font-bold uppercase tracking-widest text-sm text-[#8B5E3C]">Total Payable</span>
                    <span className="font-bold text-5xl text-[#16A34A] font-outfit">₹{grandTotal}</span>
                  </div>

                  <div className="bg-white rounded-xl p-4 border border-[#E8E1D5] mb-6">
                    <h4 className="font-bold text-[#1A4D2E] mb-3 uppercase tracking-widest text-xs">Payment Method</h4>
                    <div className="flex flex-col gap-3">
                      <label className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${paymentMethod === 'online' ? 'border-[#16A34A]' : 'border-[#C19B6C] group-hover:border-[#1A4D2E]'}`}>
                          {paymentMethod === 'online' && <div className="w-2.5 h-2.5 bg-[#16A34A] rounded-full" />}
                        </div>
                        ...
                        <span className="text-[#2C3E35] font-medium text-sm">Pay Online (Razorpay)</span>
                      </label>
                      <label className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${paymentMethod === 'cod' ? 'border-[#16A34A]' : 'border-[#C19B6C] group-hover:border-[#1A4D2E]'}`}>
                          {paymentMethod === 'cod' && <div className="w-2.5 h-2.5 bg-[#16A34A] rounded-full" />}
                        </div>
                        <input type="radio" name="paymentMethod" value="cod" checked={paymentMethod === 'cod'} onChange={() => setPaymentMethod('cod')} className="hidden" />
                        <span className="text-[#2C3E35] font-medium text-sm">Cash on Delivery / Test</span>
                      </label>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 relative z-10">
                  <button
                    onClick={handlePay}
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-3 bg-[#8B5E3C] text-white py-5 rounded-2xl font-bold text-xl hover:bg-[#734A2E] transition-all duration-300 shadow-[0_8px_25px_rgba(139,94,60,0.35)] hover:-translate-y-1 hover:shadow-[0_15px_35px_rgba(139,94,60,0.45)] disabled:opacity-50 disabled:transform-none"
                  >
                    {loading ? (
                  ...
                  <div className="flex items-center justify-center gap-2 text-[#FDFBF7]/60 text-xs font-bold uppercase tracking-widest mt-6">
                    <ShieldCheck className="w-4 h-4 text-[#8B5E3C]" /> 100% Safe Payments
                  </div>
                </div>
              </div>
            </div>
