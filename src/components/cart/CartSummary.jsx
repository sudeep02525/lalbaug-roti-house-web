  ...
  return (
    <div className="lg:col-span-5">
      <div className="bg-[#FDFBF7] border border-[#E8E1D5] rounded-[2.5rem] p-6 lg:p-8 text-[#114D3C] sticky top-24 shadow-sm relative overflow-hidden">
        ...
        <div className="mb-6 space-y-3 pb-6 border-b border-[#E8E1D5]/50">
          {items.map(item => {
            const basePrice = item.variant ? item.variant.price : item.product.price;
            const addonsPrice = (item.addons || []).reduce((addSum, add) => addSum + (add.price * (add.quantity || 1)), 0);
            ...
            return (
              <div key={item.cartItemId} className="flex justify-between items-start text-sm mb-4 last:mb-0" style={{ fontFamily: "var(--font-outfit)" }}>
                <div className="flex-1 pr-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="font-bold text-[#114D3C] text-base leading-tight">
                        {item.product.name}
                ...
                <div className="flex flex-col items-end shrink-0 pt-0.5">
                  ...
                  <span className="font-bold text-[#114D3C] text-base">₹{discountedLineTotal}</span>
                </div>
              </div>
            )
          })}
